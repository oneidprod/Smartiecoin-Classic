...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::SMARTIE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert smartiecoin:// to smartiecoin:
    //
    //    Cannot handle this later, because smartiecoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("smartiecoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "smartiecoin:");
    }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SMARTIE, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "smartiecoin.desktop";
    return GetAutostartDir() / strprintf("smartiecoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a smartiecoin.desktop file to the autostart directory:
