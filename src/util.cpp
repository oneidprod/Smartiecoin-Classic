...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "smartiecoin" is a composite category enabling all Smartiecoin-related debug output
            if(ptrCategory->count(string("smartiecoin"))) {
...
#else
    const char* pszModule = "smartiecoin";
#endif
    if (pex)
        return strprintf(
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty smartiecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override smartiecoin.conf
